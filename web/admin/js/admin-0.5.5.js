// myhistoree Admin Dashboard JS v0.5.5-hotfix
const API = '/api';
...
async function loadEncountersDashboard() {
  await Promise.all([
    loadOffeneVorgaenge(),
    loadInBearbeitung(),
    loadAbgeschlosseneVorgaenge()
  ]);
}

async function loadOffeneVorgaenge() {
  ...
  try {
    ...
    if (!pending.length) { container.innerHTML = '<div class="empty">Keine offenen Vorgaenge</div>'; return; }
    const baseUrl = window.location.origin.replace('/admin', '');
    container.innerHTML = `<table><thead><tr><th>PVS Patienten-ID</th><th>Geburtsdatum</th><th>Erstellt</th><th>Gueltig bis</th><th>Aktion</th></tr></thead><tbody>${pending.map(r => {
      const url = `${baseUrl}/anamnese/${r.token}`;
      return `<tr><td><strong>${r.pvs_patient_id || '-'}</strong></td><td>${formatDate(r.patient_dob, 'date') || '-'}</td><td>${formatDate(r.created_at, 'date')}</td><td>${formatDate(r.expires_at, 'date')}</td><td><button class="btn btn-sm" onclick="showQRModal('${url}', 'QR – ${r.pvs_patient_id || '?'}')">QR</button> <button class="btn btn-sm btn-danger" onclick="deleteLink('${r.token}')">Loeschen</button></td></tr>`;
    }).join('')}</tbody></table>`;
  } catch(e) { container.innerHTML = '<div class="empty">Fehler: ' + e.message + '</div>'; }
}

async function loadInBearbeitung() {
  ...
  try {
    ...
    if (!completed.length) { container.innerHTML = '<div class="empty">Keine Vorgaenge in Bearbeitung</div>'; return; }
    container.innerHTML = `<table><thead><tr><th>PVS Patienten-ID</th><th>Abgeschickt am</th><th>Aktion</th></tr></thead><tbody>${completed.map(r => {
      return `<tr><td><strong>${r.pvs_patient_id || '-'}</strong></td><td><span class="badge badge-completed">${formatDate(r.completed_at || r.created_at, 'date')} ${new Date(r.completed_at || r.created_at).toLocaleTimeString('de-DE', {hour:'2-digit', minute:'2-digit', timeZone: 'Europe/Berlin'})}</span></td><td><button class="btn btn-sm" onclick="viewEncounter('${r.id}', '${r.pvs_patient_id || ''}')">Ansehen</button> <button class="btn btn-sm btn-success" onclick="printEncounter('${r.id}', '${r.pvs_patient_id || ''}')">Drucken</button> <button class="btn btn-sm btn-primary" onclick="processEncounter('${r.id}')">Fertig</button></td></tr>`;
    }).join('')}</tbody></table>`;
  } catch(e) { container.innerHTML = '<div class="empty">Fehler: ' + e.message + '</div>'; }
}

async function loadAbgeschlosseneVorgaenge() {
  ...
  try {
    ...
    const processed = rows.filter(r => r.status === 'processed' && new Date(r.processed_at || r.updated_at) >= sevenDaysAgo);
    if (!processed.length) { container.innerHTML = '<div class="empty">Keine abgeschlossenen Vorgaenge in den letzten 7 Tagen</div>'; return; }
    container.innerHTML = `<table><thead><tr><th>PVS Patienten-ID</th><th>Verarbeitet am</th><th>Aktion</th></tr></thead><tbody>${processed.map(r => {
      return `<tr><td><strong>${r.pvs_patient_id || '-'}</strong></td><td><span class="badge badge-processed">${formatDate(r.processed_at || r.updated_at, 'date')} ${new Date(r.processed_at || r.updated_at).toLocaleTimeString('de-DE', {hour:'2-digit', minute:'2-digit', timeZone: 'Europe/Berlin'})}</span></td><td><button class="btn btn-sm" onclick="viewEncounter('${r.id}', '${r.pvs_patient_id || ''}')">Ansehen</button></td></tr>`;
    }).join('')}</tbody></table>`;
  } catch(e) { container.innerHTML = '<div class="empty">Fehler: ' + e.message + '</div>'; }
}
