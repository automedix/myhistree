...
function copyLink(token, docType) {
  const path = docType === 'consent_form' ? 'auffklaerung' : 'anamnese';
  ...
  try {
    ...
    if (encData.encounter && encData.encounter.document_type === 'consent_form') {
      ...
      if (enc.signature_svg) {
        ...
        html += `<div style="border:1px solid var(--border);border-radius:8px;padding:12px;background:#fafafa;">${enc.signature_svg}</div>`;
      }
  ...
  try {
    ...
    if (enc.document_type === 'consent_form') {
      ...
      if (cData.encounter?.signature_svg) {
        ...
        html += `<div style="border:1px solid #ccc;padding:12px;">${cData.encounter.signature_svg}</div>`;
      }
      html += '</div>';
