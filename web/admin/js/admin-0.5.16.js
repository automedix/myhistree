...
function renderQuoteEditor() {
  ...
  html += '<div><label>E-Mail</label><input type="text" id="q-email" value="' + escapeHtml(d.patientEmail) + '" autocomplete="off" oninput="QUOTE_DRAFT.patientEmail = this.value" onchange="QUOTE_DRAFT.patientEmail = this.value"></div>';
  html += '</div>';
...
if (isDraft) {
  ...
  html += 'Hinweis: Dieser Kostenvoranschlag ist nach GOÄ mit einem ' + d.multiplier + '-fachen Steigerungssatz kalkuliert. Die tatsächlichen Kosten koennen je nach Aufwand abweichen. Gueltigkeit: 7 Tage. Die Krankenkasse uebernimmt diese Leistungen nicht.';
  html += '</div>';
...
function renderQuoteItemsRows() {
  ...
  if (!items.length) return '<tr><td colspan="6" style="text-align:center;color:#888;">Keine Positionen. Suchen Sie GOÄ-Ziffern oder waehlen Sie ein Template.</td></tr>';
  let html = '';
...
async function recalcQuoteItems() {
  ...
  if (hintDiv) {
    hintDiv.textContent = 'Hinweis: Dieser Kostenvoranschlag ist nach GOÄ mit einem ' + mult + '-fachen Steigerungssatz kalkuliert. Die tatsächlichen Kosten koennen je nach Aufwand abweichen. Gueltigkeit: 7 Tage. Die Krankenkasse uebernimmt diese Leistungen nicht.';
  }
}
...
async function saveQuote(finalize) {
  ...
  if (emailEl) {
    const domEmail = emailEl.value.trim();
    if (domEmail || !d.patientEmail) d.patientEmail = domEmail;
    console.log('DEBUG saveQuote email:', { domEmail, draftEmail: d.patientEmail });
  }
  ...
  try {
    ...
    console.log('PUT response OK: ', res2.status);
    ...
    if (finalize) { await loadQuotes(); } else { const btn = document.activeElement; if (btn && btn.textContent && btn.textContent.includes('Entwurf')) { btn.textContent = 'Gespeichert!'; setTimeout(() => btn.textContent = 'Speichern (Entwurf)', 1500); } else if (btn && btn.textContent && btn.textContent.includes('Aktualisieren')) { btn.textContent = 'Gespeichert!'; setTimeout(() => btn.textContent = 'Aktualisieren', 1500); } else { alert('Gespeichert!'); } }
  } catch (e) { alert('Fehler: ' + (e.message || e)); }
}
...
async function sendQuoteLink() {
  ...
  try {
    ...
    if (emailEl2) {
      const domEmail = emailEl2.value.trim();
      if (domEmail || !QUOTE_DRAFT.patientEmail) QUOTE_DRAFT.patientEmail = domEmail;
      console.log('DEBUG sendLink email:', { domEmail, draftEmail: QUOTE_DRAFT.patientEmail });
    }
